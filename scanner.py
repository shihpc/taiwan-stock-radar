...
def run_scan(scan_date: str = None, quick: bool = False,
    ...
    broker_cache = None
    if use_broker:
        from engine.broker_analysis import BrokerDataCache
        from data.fetcher import fetch_broker_data
        broker_cache = BrokerDataCache()
        logger.info("分點快取已初始化（Sponsor 模式）")
    ...
    for i, (_, stock_row) in enumerate(valid_stocks.iterrows()):
        ...
        try:
            ...
            if use_broker and broker_cache is not None:
                # 拉取近 10 日分點
                bt_dates = [
                ...
                broker_df = broker_cache.get_multi_days(
                    stock_id, bt_dates, fetch_broker_data
                )
